...
Web `/trading/backtest` で、スプレッド/スリッページを差し引いた検証ができます。
- **直近60日（15分足）** … 無料のリアル価格（Yahoo）。口座不要
- **長期（取り込み済み・15分足）** … HistData.com の長期M1を取り込み、
  **同じ15分足ロジックのまま数年ぶん**を検証

長期データの取り込み（サーバーで一度だけ）:
```bash
# 自動DL（ベストエフォート）
...
```
取り込み後、`/trading/backtest` の期間で「長期」を選べます（`trading/histdata.py`、
`HIST_DB_PATH` に保存）。
